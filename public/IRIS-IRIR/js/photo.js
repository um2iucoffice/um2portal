...
function populateIDCard(s, isGraduated, gradID, gradDate) {
  ...
  // QR code is generated by generateIDCardQR() in qr.js (called from populate.js).
  // That function fetches a real signed token from the server and renders the QR,
  // then stores the URL in window._idcardVerifyUrl for printIDCard() to use.
  // Do NOT generate a fallback QR here — it would use a plain /verify?id= URL
