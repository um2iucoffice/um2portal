...
.section-sep{height:1px;background:var(--line);margin:40px 0 0;}
...
@media print {
  ...
  * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  #loginScreen, .header, .nav,
  .ql, .qlinks, .btn-out, .btn-login, footer { display: none !important; }
...
.cert-banner {
  background: linear-gradient(135deg, var(--gold-light) 0%, #fff8e8 60%, var(--gold-light) 100%);
  border: 1.5px solid rgba(154,123,47,.3);
  border-radius: 10px;
  ...
  gap: 0;
  box-shadow: 0 2px 12px rgba(154,123,47,.10);
  transition: border-color .15s, box-shadow .15s;
}
.cert-banner:hover { border-color: var(--gold); box-shadow: 0 6px 24px rgba(154,123,47,.18); }
.cert-banner::before {
  content: '';
  position: absolute;
...
.cert-banner .doc-icon { background: var(--gold-light); border: 1px solid rgba(154,123,47,.2); }
.cert-banner .doc-icon svg { stroke: var(--gold); }
.cert-banner .doc-title { color: var(--gold); }
.cert-banner .doc-tag { border-color: rgba(154,123,47,.3); color: var(--gold); background: #fff8e8; }
.cert-banner .btn-print-doc { background: var(--gold); }
.cert-banner .btn-print-doc:hover { background: #7a6122; }
.cert-section-head {
  font-size: 11px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase;
  color: var(--gold); margin: 24px 0 12px; display: flex; align-items: center; gap: 8px;
}
.cert-section-head::after { content: ''; flex: 1; height: 1px; background: rgba(154,123,47,.2); }
...
@media(max-width:700px){
  ...
  .banner{grid-template-columns:1fr;gap:12px}
  .igrid{grid-template-columns:1fr}
  .icell:nth-child(odd){border-right:none}
  .icell:nth-last-child(-n+2){border-bottom:1px solid var(--line)}
...
<div id="dashboard">
  ...
  <div class="header">
    <div class="header-inner">
      <div class="h-brand">
        <img src="https://raw.githubusercontent.com/um2iucoffice/um2portal/refs/heads/main/public/UM2_Logo.svg" alt="UM2 Logo" style="height:32px;width:auto;object-fit:contain;">
        <div class="h-divider"></div>
        <span class="h-name"> IRIS System</span>
      </div>
      <div class="h-user"><span>Signed in as</span><span id="topbarEmail">—</span></div>
      <button class="btn-out" onclick="doSignOut()">Sign out</button>
    </div>
  </div>

  <div class="nav" style="position:sticky;top:58px;z-index:49">
    <div class="nav-inner">
      <button class="ntab active" id="navHome"    onclick="scrollToSection('panel-home',this)">Home</button>
      <button class="ntab" id="navProfile"        onclick="scrollToSection('panel-profile',this)">My Personal Page</button>
      <button class="ntab" id="navGrades"         onclick="scrollToSection('panel-grades',this)">Academic Record</button>
      <button class="ntab" id="navDocs"           onclick="scrollToSection('panel-docs',this)">Document Office</button>
      <button class="ntab" id="navIdCard"         onclick="scrollToSection('panel-idcard',this)">ID Card</button>
      <button class="ntab" id="navPhoto"          onclick="scrollToSection('panel-photo',this)">Photo</button>
      <button class="ntab ext" onclick="window.open('https://um2campus.com/login/index.php','_blank')">E-Learning</button>
      <button class="ntab ext" onclick="window.open('https://supportportal.um2campus.org','_blank')">Support</button>
    </div>
  </div>
  ...
  <div class="main">
    ...
    <div class="panel active" id="panel-home">
      ...
      <div class="qlinks">
        ...
        <div class="ql" onclick="switchTab('idcard')">
          ...
          <div class="ql-t">Student ID Card</div>
    ...
    <div class="panel" id="panel-profile">
      <div class="ph"><h2>My Personal Page</h2><p>Personal &amp; enrollment information</p></div>
      <div class="banner">
      ...
      <div class="icard">
        ...
        <div class="igrid">
          ...
          <div class="icell" id="cellGradID" style="display:none"><div class="ic-lbl">Graduation ID</div><div class="ic-val" id="infoGradID">—</div></div>
      ...
      <!-- Multi-program enrollment detail — shown when 2+ enrollments -->
      <div id="multiProgramSection" style="display:none;margin-top:16px">
        <div class="icard">
          <div class="icard-head">All Degree Programs</div>
          <div id="multiProgramRows"><!-- populated by renderMultiProgramProfile() --></div>
        </div>
      </div>
    </div>
    ...
    <div class="panel" id="panel-docs">
      ...
      <!-- Degree picker — shown when student has 2+ enrollments -->
      <div class="doc-degree-picker" id="docDegreePicker">
        <label>Select Degree Program to generate document for:</label>
        <select id="docDegreeSelect" onchange="onDocDegreeChange()"><!-- populated dynamically --></select>
      </div>
      ...
      <div class="doc-grid">
        ...
        <div class="doc-card">
          ...
          <div class="doc-title" id="docConfirmTitle">Confirmation of Study</div>
          <div class="doc-desc" id="docConfirmDesc">Official letter confirming your current enrollment status at the University of Medicine (2).</div>
          <div class="doc-tags">
            <span class="doc-tag">Official Document</span>
            <span class="doc-tag">Instant</span>
          </div>
          <button class="btn-print-doc" onclick="printDocument('confirmation')">
        ...
        <div class="doc-card">
          ...
          <div class="doc-desc">Complete record of your academic performance including all course grades and scores.</div>
          <div class="doc-tags">
            <span class="doc-tag">Official Document</span>
            <span class="doc-tag">Instant</span>
          </div>
    ...
    <!-- ID CARD -->
    <div class="section-sep" aria-hidden="true"></div>
    <div class="panel" id="panel-idcard">
      <div class="ph"><h2 id="idCardPageTitle">Student ID Card</h2><p id="idCardPageSub">Your official University of Medicine (2) student identification</p></div>
    ...
    <div class="panel" id="panel-grades">
      ...
      <div class="grades-header-banner" id="gradesHeaderBanner" style="display:none">
        ...
        <div class="grades-hdr-gpa">
          <div class="grades-hdr-gpa-label">Cumulative GPA</div>
          <div class="grades-hdr-gpa-val" id="gradesGpa">—</div>
          <div class="grades-hdr-gpa-den">/ 4.00</div>
        </div>
      </div>
  ...
  <!-- footer -->
  <footer style="border-top:1px solid var(--line);padding:18px 28px;text-align:center;font-size:11px;color:var(--ink3);background:rgba(255,255,255,0.7);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);">
...
function populate(s, grades, courses, program_meta, enrollments) {
  ...
  // ── Document office degree picker ────────────────────────────
  renderDocDegreePicker(window._enrollments);

  // ── Degree Certificate cards ─────────────────────────────────
  renderCertCards(window._enrollments);

  // ── Multi-program profile section ───────────────────────────
  renderMultiProgramProfile(window._enrollments);
...
function scrollToSection(panelId, btn) {
...
// Legacy switchTab kept for internal calls (e.g. from quick-link cards)
function switchTab(id, btn) {
  scrollToSection('panel-' + id, btn);
}

// ── IntersectionObserver: highlight nav as user scrolls ──────
(function initScrollSpy() {
  function ready() {
    const panels = ['panel-home','panel-profile','panel-grades','panel-docs','panel-idcard','panel-photo'];
    const navMap = {
      'panel-home':    'navHome',
      'panel-profile': 'navProfile',
      'panel-grades':  'navGrades',
      'panel-docs':    'navDocs',
      'panel-idcard':  'navIdCard',
      'panel-photo':   'navPhoto',
    };
...
function printDocument(type) {
  ...
  if (type === 'transcript') {
    ...
    gradeContainer.querySelectorAll('.g-wrap').forEach(wrap => {
      ...
      const year = head?.dataset?.year || (head?.textContent || '').replace(/^—\s*/, '').replace(/\s*·\s*MBBS Program\s*$/i, '').trim() || 'Academic Year';
      const rows = [];
  ...
  // ── Helper: render one semester table ────────────────────────
  const TNR = "'DM Sans', sans-serif";
    ...
    return `
      <div style="margin-bottom:22px;page-break-inside:avoid">
        <div style="font-family:'DM Sans',sans-serif;font-size:13pt;font-weight:bold;color:#0D1B2A;margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid #0D1B2A;display:flex;justify-content:space-between;align-items:baseline">
          <span>— &nbsp;${group.year} &nbsp;·&nbsp; MBBS Program</span>
        </div>
...
const confirmationBody = type === 'confirmation' ? `
  ...
  ">
    ...
    <p>
      ...
      for the degree of <strong>Bachelor of Medicine, Bachelor of Surgery (MBBS)</strong>
  ...
  with the enrollment status - <strong>${enroll}</strong>.
    ...
    <br>
    <p>
      This letter is issued digitally for official purposes.
    </p>
  </div>` : '';
...
const closing = `
  <div style="margin-top:28px;border-top:1.5px solid #0D1B2A;display:flex;justify-content:space-between;align-items:stretch;font-family:'DM Sans',sans-serif">
    ...
    <!-- Official Notice -->
    <div style="flex:1;padding:12px 16px;border-right:0px solid #ccc;font-size:11pt;color:#0D1B2A;line-height:1.7;font-family:'DM Sans',sans-serif">
      <div style="font-size:11pt;font-weight:bold;letter-spacing:.8px;text-transform:uppercase;text-align:justify;color:#0D1B2A;margin-bottom:5px">
        Official Notice
      </div>

<div class="tsc-official-record">
  ...
  <div style="display:flex;align-items:center;gap:16px;margin-top:8px;">
    ...
    <img 
      ...
      alt="Registrar Signature"
       style="width:180px;height:auto;object-fit:contain;position:relative;top:-50px;"
    >
  </div>
  </div>`;
...
<head>
  <meta charset="UTF-8">
  <title>${type === 'transcript' ? 'Academic Transcript' : (isGraduated ? 'Confirmation of Graduation' : 'Confirmation of Study')} — ${name}</title>
...
.wm {
  ...
  z-index: 0;
    ...
    .inst-hdr {
      ...
      margin-bottom: 12px; padding-bottom: 10px; border-bottom: 2.5px solid #0D1B2A;
    }
    .inst-name {
      font-family: 'DM Sans', sans-serif;
      font-size: 17pt; font-weight: bold; color: #0D1B2A; line-height: 1.2;
    }
    .inst-sub { font-size: 9.5pt; color: #555; margin-top: 4px; font-family: 'DM Sans', sans-serif; font-style: italic; }
    .doc-type-badge {
      font-family: 'DM Sans', sans-serif;
      font-size: 13pt; font-weight: bold; color: #0D1B2A;
    ...
    .stu-bar {
      ...
      font-size: 9pt; color: #333; font-family: 'DM Sans', sans-serif;
    }
    .stu-bar-item { display: flex; flex-direction: column; gap: 2px; }
    .stu-bar-lbl { font-size: 7.5pt; font-weight: bold; letter-spacing: .5px; text-transform: uppercase; color: #666; }
    .stu-bar-val { font-size: 9.5pt; color: #0D1B2A; font-weight: bold; }
    ...
    .section-label {
      font-size: 8pt; font-weight: bold; letter-spacing: 1.2px;
      text-transform: uppercase; color: #444; font-family: 'DM Sans', sans-serif;
      margin: 14px 0 6px; border-bottom: 1px solid #ccc; padding-bottom: 4px;
    }
  </style>
</head>
<body>
  ...
  <table>
    ...
    <tbody><tr><td style="position:relative;z-index:1">
      ...
      <div class="inst-hdr">
        ...
        <div>
          <div class="doc-type-badge">${type === 'transcript' ? 'Official Transcript' : (isGraduated ? 'Confirmation of Graduation' : 'Confirmation of Study')}</div>
          <div class="doc-meta">
      ...
      <div class="transcript-student-section" style="grid-template-columns:1.2fr 1.2fr .8fr ${program && program !== '—' ? '.8fr' : ''} ${isGraduated && gradID ? '.8fr' : ''}">
        ...
        ${program && program !== '—' ? `<div class="transcript-student-item"><div class="transcript-student-label">Degree Program</div><div class="transcript-student-value">${program}</div></div>` : ''}
        ${isGraduated && gradID ? `<div class="transcript-student-item"><div class="transcript-student-label">Graduation ID</div><div class="transcript-student-value">${gradID}</div></div>` : ''}
      </div>
      ${isGraduated ? `<div class="section-label">Graduation Information</div>
      <div class="transcript-student-section" style="grid-template-columns:1fr 1fr">
        ...
        ${gradDate ? `<div class="transcript-student-item"><div class="transcript-student-label">Date of Graduation</div><div class="transcript-student-value">${gradDate}</div></div>` : ''}
      </div>` : ''}
      <div class="section-label">Academic Record</div>${academicRecord}` : ''}
      ${confirmationBody}
      ${closing}
...
// ── Watermark helper ─────────────────────────────────────────
// Returns a Promise<Blob> of the image with a tiled "UM2" watermark
function applyWatermark(sourceDataUrl) {
  return new Promise(function(resolve) {
    ...
    img.onload = function() {
      ...
      // Watermark style
      const text     = 'UM2';
      const fontSize = Math.max(12, Math.round(canvas.width * 0.055));
      ctx.font        = 'bold ' + fontSize + 'px DM Sans, Arial, sans-serif';
      ctx.fillStyle   = 'rgba(139, 26, 46, 0.20)';
      ctx.textBaseline = 'middle';

      // Tile across the whole image at a diagonal
      const stepX = fontSize * 4.2;
      const stepY = fontSize * 2.6;
      const angle = -22 * Math.PI / 180;

      ctx.save();
      // Extend tiling beyond canvas edges to cover rotated corners
...
function previewPhoto(input) {
  ...
  reader.onload = function(e) {
    // Apply watermark and show watermarked preview
    applyWatermark(e.target.result).then(function(wmBlob) {
      const wmUrl = URL.createObjectURL(wmBlob);

      // Store watermarked blob on the input element for upload
      input._watermarkedBlob = wmBlob;

      // Update current photo preview card
      const preview = document.getElementById('profilePhotoPreview');
      if (preview) preview.innerHTML = '<img src="' + wmUrl + '" style="width:100%;height:100%;object-fit:cover;border-radius:8px">';
      // Show selected-file info row
      ...
      if (thumb) thumb.src = wmUrl;
      if (name)  name.textContent = file.name;
      if (size)  size.textContent = (file.size / 1024).toFixed(0) + ' KB · ' + file.type.replace('image/', '').toUpperCase() + ' · Watermarked';
...
(function initDropZone() {
  function ready() {
    ...
    zone.addEventListener('dragover', function(e) {
      ...
      zone.style.background  = 'var(--crimson-light)';
    });
    ...
    zone.addEventListener('drop', function(e) {
      ...
      if (e.dataTransfer.files.length) {
        // Assign dropped file to the input and trigger preview
...
async function uploadPhoto() {
  ...
  const sid      = document.getElementById('infoID').textContent.trim();
  const password = window._sessionPassword || '';

  // Use the already-watermarked blob if available, otherwise watermark now
  const photoBlob = input._watermarkedBlob || await applyWatermark(
  ...
  formData.append('photo',     photoBlob, sid + '.jpg');   // always named by student ID

  try {
    const res  = await fetch('/.netlify/functions/update-profile', { method: 'POST', body: formData });
    const data = await res.json();
    if (data.success) {
      statusEl.textContent = '✓ Photo uploaded successfully.';
      statusEl.style.color = 'var(--green)';
      // Update avatar and ID card photo
...
async function removePhoto() {
  ...
  try {
    const res  = await fetch('/.netlify/functions/update-profile', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ studentId: sid, password, removePhoto: true })
    });
    const data = await res.json();
    if (data.success) {
      statusEl.textContent = '✓ Photo removed.';
      statusEl.style.color = 'var(--green)';
      // Reset all photo UI back to initials/placeholder
      const init = document.getElementById('avatarInitials').textContent || '?';
      ...
      if (idPhoto) { idPhoto.innerHTML = ''; idPhoto.textContent = init; }
      const homeAv = document.getElementById('homeAvatar');
      if (homeAv) homeAv.innerHTML = init;
      const gradesAv = document.getElementById('gradesAvatar');
      if (gradesAv) gradesAv.innerHTML = init;
      const preview = document.getElementById('profilePhotoPreview');
      if (preview) preview.innerHTML = '<svg width="56" height="64" viewBox="0 0 56 64" fill="none" xmlns="http://www.w3.org/2000/svg"><ellipse cx="28" cy="20" rx="13" ry="14" fill="#D1D5DB"/><path d="M2 58c0-13.255 11.64-24 26-24s26 10.745 26 24" fill="#D1D5DB"/></svg>';
      btn.style.display = 'none';
    } else {
      statusEl.textContent = data.message || 'Failed to remove photo.';
      statusEl.style.color = 'var(--crimson)';
    }
  } catch (err) {
    statusEl.textContent = 'Connection error. Please try again.';
...
function updatePhotoInUI(photoUrl) {
  ...
  const idPhoto = document.getElementById('idcardPhoto');
  if (idPhoto) { idPhoto.innerHTML = '<img src="' + photoUrl + '" style="width:100%;height:100%;object-fit:cover">'; }
  // Update home hero avatar
  const homeAv = document.getElementById('homeAvatar');
  if (homeAv) { homeAv.innerHTML = '<img src="' + photoUrl + '" style="width:100%;height:100%;object-fit:cover;border-radius:10px">'; }
  // Update grades avatar
  const gradesAv = document.getElementById('gradesAvatar');
  if (gradesAv) { gradesAv.innerHTML = '<img src="' + photoUrl + '" style="width:100%;height:100%;object-fit:cover;border-radius:8px">'; }
  // Update profile photo preview & show remove button
  const preview = document.getElementById('profilePhotoPreview');
  if (preview) { preview.innerHTML = '<img src="' + photoUrl + '" style="width:100%;height:100%;object-fit:cover;border-radius:8px">'; }
  const removeBtn = document.getElementById('removePhotoBtn');
  if (removeBtn) removeBtn.style.display = 'inline-block';
}
...
// ── Photo protection ──────────────────────────────────────────
// Renders photo into a canvas so there's no <img src> to open in a new tab.
// Also blocks right-click, drag, and long-press on all photos.
function loadPhotoToCanvas(canvasEl, url) {
  if (!canvasEl) return;
  const img = new Image();
...
function makeProtectedPhoto(url, style) {
  const canvas = document.createElement('canvas');
  canvas.style.cssText = style || 'width:100%;height:100%;object-fit:cover;display:block';
  canvas.oncontextmenu = function(e) { e.preventDefault(); return false; };
  canvas.ondragstart   = function(e) { e.preventDefault(); return false; };
  canvas.onauxclick    = function(e) { e.preventDefault(); return false; };
  canvas.onmousedown   = function(e) { if (e.button === 1) { e.preventDefault(); return false; } };
  loadPhotoToCanvas(canvas, url);
  return canvas;
}

// Override updatePhotoInUI to use canvas instead of <img>
const _origUpdatePhotoInUI = updatePhotoInUI;
function updatePhotoInUI(photoUrl) {
  // Banner avatar
  const avatarEl = document.getElementById('avatarEl');
  const initials = document.getElementById('avatarInitials');
  if (avatarEl) {
    let c = avatarEl.querySelector('canvas.photo-canvas');
    if (!c) { c = makeProtectedPhoto(photoUrl, 'width:100%;height:100%;object-fit:cover;border-radius:6px;display:block'); c.className = 'photo-canvas'; avatarEl.prepend(c); }
    else loadPhotoToCanvas(c, photoUrl);
  }
  if (initials) initials.style.display = 'none';
  const avatarPhoto = document.getElementById('avatarPhoto');
  ...
  // ID card photo
  const idPhoto = document.getElementById('idcardPhoto');
  if (idPhoto) { idPhoto.innerHTML = ''; idPhoto.appendChild(makeProtectedPhoto(photoUrl, 'width:100%;height:100%;object-fit:cover;display:block')); }

  // Home hero avatar
  const homeAv = document.getElementById('homeAvatar');
  if (homeAv) { homeAv.innerHTML = ''; homeAv.appendChild(makeProtectedPhoto(photoUrl, 'width:100%;height:100%;object-fit:cover;border-radius:10px;display:block')); }

  // Grades avatar
  const gradesAv = document.getElementById('gradesAvatar');
  if (gradesAv) { gradesAv.innerHTML = ''; gradesAv.appendChild(makeProtectedPhoto(photoUrl, 'width:100%;height:100%;object-fit:cover;border-radius:8px;display:block')); }

  // Profile photo preview
  const preview = document.getElementById('profilePhotoPreview');
  if (preview) { preview.innerHTML = ''; preview.appendChild(makeProtectedPhoto(photoUrl, 'width:100%;height:100%;object-fit:cover;border-radius:8px;display:block')); }
...
// Block right-click and drag on the whole page for images/canvas
...
// Block middle-click ("open in new tab") on images/canvas
document.addEventListener('auxclick', function(e) {
  if (e.button === 1 && (e.target.tagName === 'IMG' || e.target.tagName === 'CANVAS')) {
    e.preventDefault(); return false;
  }
});
...
// ══════════════════════════════════════════
// MULTI-DEGREE HELPERS
// ══════════════════════════════════════════

const LEVEL_ICON  = { bachelor:'🎓', master:'📖', phd:'⚗️' };
const LEVEL_LABEL = { bachelor:'Bachelor', master:'Master', phd:'PhD / Doctoral' };
const LEVEL_CSS   = { bachelor:'bach', master:'mast', phd:'phd' };

function levelCss(lvl) { return LEVEL_CSS[lvl] || 'bach'; }
function levelLabel(lvl) { return LEVEL_LABEL[lvl] || 'Bachelor'; }
function levelIcon(lvl)  { return LEVEL_ICON[lvl]  || '🎓'; }

function enrollmentBadge(e) {
  const g = (e.graduationStatus || '').toLowerCase();
  if (/graduated|completed/.test(g)) return '<span class="dbadge dbadge-green">Graduated</span>';
  const st = (e.enrollmentStatus || '').toLowerCase();
  if (st === 'active')    return '<span class="dbadge dbadge-gold">Active</span>';
  if (st === 'deferred')  return '<span class="dbadge dbadge-muted">Deferred</span>';
  if (st === 'withdrawn') return '<span class="dbadge dbadge-red">Withdrawn</span>';
  return '<span class="dbadge dbadge-muted">' + (e.enrollmentStatus || '—') + '</span>';
}
...
function renderJourney(enrollments) {
  ...
  if (!card || !rows) return;
  // Only show if more than one enrollment exists
  if (!enrollments || enrollments.length < 2) { card.style.display = 'none'; return; }
  card.style.display = '';
  rows.innerHTML = enrollments.map((e, i) => {
    const lvl = levelCss(e.degreeLevel);
    const gpaStr = e.gpa ? e.gpa : '—';
    const isGrad = /graduated|completed/i.test(e.graduationStatus || '');
    let meta = e.currentYear || '';
    if (e.admissionDate) meta += (meta ? ' · ' : '') + 'Admitted ' + e.admissionDate;
    if (isGrad && e.graduationDate) meta += ' · Graduated ' + e.graduationDate;
    if (e.thesisTitle) meta += '<br><em style="color:var(--ink3);font-size:11px">"' + e.thesisTitle + '"</em>';
    if (e.supervisor)  meta += '<br><span style="font-size:11px;color:var(--ink3)">Supervisor: ' + e.supervisor + '</span>';
    return `<div class="degree-row">
      <div class="degree-lvl-pip ${lvl}"></div>
      <div class="degree-row-icon ${lvl}">${levelIcon(e.degreeLevel)}</div>
      <div class="degree-row-info">
        <div class="degree-row-lvl ${lvl}">${levelLabel(e.degreeLevel)}</div>
        <div class="degree-row-name">${e.programName || e.degreeProgramId || '—'}</div>
        <div class="degree-row-meta">${meta}</div>
      </div>
      <div class="degree-row-badge">${enrollmentBadge(e)}</div>
      <div class="degree-row-gpa">
        <div class="degree-row-gpa-val" style="color:var(--${lvl})">${gpaStr}</div>
        <div class="degree-row-gpa-den">/ 4.00</div>
      </div>
    </div>`;
  }).join('');
}
...
function renderDegreeTabs(enrollments, grades, courses) {
  const tabsWrap = document.getElementById('gradeDegreeTabs');
  const tabsBar  = document.getElementById('degreeTabsBar');
  if (!tabsWrap || !tabsBar) return;
  if (!enrollments || enrollments.length < 2) {
    tabsWrap.style.display = 'none';
    // Just render grades normally (all grades, no filter)
    renderGrades(grades, courses);
    return;
  }
  tabsWrap.style.display = '';
  // Store grades + courses globally for tab switching
  window._allGrades  = grades;
  window._allCourses = courses;

  tabsBar.innerHTML = enrollments.map((e, i) => {
    const lvl = levelCss(e.degreeLevel);
    return `<button class="degree-tab${i===0?' active':''}" onclick="switchDegreeTab(${i})" id="dtab-${i}">
      ${levelIcon(e.degreeLevel)} ${e.programName || e.degreeProgramId || 'Degree ' + (i+1)}
    </button>`;
  }).join('');

  // Render grades for the first tab
  renderGradesForEnrollment(0, enrollments, grades, courses);
}

function switchDegreeTab(idx) {
  document.querySelectorAll('.degree-tab').forEach((t, i) => {
    t.classList.toggle('active', i === idx);
  });
  window._activeEnrollmentIndex = idx;
  renderGradesForEnrollment(idx, window._enrollments, window._allGrades, window._allCourses);
}

function renderGradesForEnrollment(idx, enrollments, grades, courses) {
  const e = enrollments[idx];
  if (!e) { renderGrades(grades, courses); return; }
  // Filter grades by enrollment_id if available, else show all for single-degree
  let filtered = grades;
  if (e.id && grades.some(g => g.enrollmentId)) {
    filtered = grades.filter(g => !g.enrollmentId || g.enrollmentId === e.id);
  }
  renderGrades(filtered, courses);
}

// ── Document degree picker ───────────────────────────────────
function renderDocDegreePicker(enrollments) {
  const picker = document.getElementById('docDegreePicker');
  const sel    = document.getElementById('docDegreeSelect');
  ...
  sel.innerHTML = enrollments.map((e, i) => {
    const lvl = levelLabel(e.degreeLevel);
    const status = /graduated|completed/i.test(e.graduationStatus || '') ? ' (Graduated)' : ' (Active)';
    return `<option value="${i}">${levelIcon(e.degreeLevel)} ${lvl} — ${e.programName || e.degreeProgramId}${status}</option>`;
  }).join('');
  window._docEnrollmentIndex = 0;
}

// ── Multi-program profile rows ───────────────────────────────
function renderMultiProgramProfile(enrollments) {
  const section = document.getElementById('multiProgramSection');
  const rows    = document.getElementById('multiProgramRows');
  if (!section || !rows) return;
  if (!enrollments || enrollments.length < 2) { section.style.display = 'none'; return; }
  section.style.display = '';
  const LEVEL_CSS   = { bachelor:'bach', master:'mast', phd:'phd' };
  const LEVEL_LABEL = { bachelor:'Bachelor', master:'Master', phd:'PhD / Doctoral' };
  const LEVEL_ICON  = { bachelor:'🎓', master:'📖', phd:'⚗️' };
  rows.innerHTML = enrollments.map((e, i) => {
    const lvl     = LEVEL_CSS[e.degreeLevel] || 'bach';
    const lbl     = LEVEL_LABEL[e.degreeLevel] || 'Bachelor';
    const icon    = LEVEL_ICON[e.degreeLevel] || '🎓';
    const isGrad  = /graduated|completed/i.test(e.graduationStatus || '');
    const gpaStr  = e.gpa || '—';
    const status  = isGrad
      ? '<span class="dbadge dbadge-green">Graduated</span>'
      : '<span class="dbadge dbadge-gold">' + (e.enrollmentStatus || 'Active') + '</span>';
    return `<div class="degree-row" style="border-bottom:${i < enrollments.length-1 ? '1px solid var(--line)' : 'none'}">
      <div class="degree-lvl-pip ${lvl}"></div>
      <div class="degree-row-icon ${lvl}">${icon}</div>
      <div class="degree-row-info">
        <div class="degree-row-lvl ${lvl}">${lbl}</div>
        <div class="degree-row-name">${e.programName || e.degreeProgramId || '—'}</div>
        <div class="degree-row-meta">
          ${e.currentYear ? e.currentYear : ''}
          ${e.admissionDate ? ' · Admitted ' + e.admissionDate : ''}
          ${isGrad && e.graduationDate ? ' · Graduated ' + e.graduationDate : ''}
          ${e.thesisTitle ? '<br><em style="color:var(--ink3);font-size:11px">"' + e.thesisTitle + '"</em>' : ''}
          ${e.supervisor  ? '<br><span style="font-size:11px;color:var(--ink3)">Supervisor: ' + e.supervisor + '</span>' : ''}
          ${isGrad && e.graduationId ? '<br><span style="font-size:11px;color:var(--green);font-family:monospace">Graduation ID: ' + e.graduationId + '</span>' : ''}
        </div>
      </div>
      <div class="degree-row-badge">${status}</div>
      <div class="degree-row-gpa">
        <div class="degree-row-gpa-val" style="color:var(--${lvl})">${gpaStr}</div>
        <div class="degree-row-gpa-den">/ 4.00</div>
      </div>
...
function onDocDegreeChange() {
  const sel = document.getElementById('docDegreeSelect');
  if (sel) window._docEnrollmentIndex = parseInt(sel.value, 10) || 0;
}

// ── Degree Certificate cards ─────────────────────────────────
// Renders one certificate card per graduated degree enrollment.
// Called after populate() whenever enrollments are available.
function renderCertCards(enrollments) {
  ...
  grid.innerHTML = graduated.map((e, i) => {
    const progName  = e.programName || e.degreeProgramId || 'Degree Program';
    const lvlLabel  = { bachelor:'Bachelor', master:'Master', phd:'PhD / Doctoral' }[e.degreeLevel] || 'Bachelor';
    const gradDate  = e.graduationDate || '';
    const gradID    = e.graduationId   || '';
    const multiProg = graduated.length > 1;
    // Find the index in _enrollments for printDocument to use
    const enrollIdx = (window._enrollments || []).indexOf(e);
    const printIdx  = enrollIdx >= 0 ? enrollIdx : 0;

    return `<div class="cert-banner doc-card">
      <div class="doc-icon">
        <svg viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
          <circle cx="12" cy="8" r="6"/>
          <path d="M9 12l2 2 4-4"/>
          <path d="M6 20l6-3 6 3V14a6 6 0 0 0-12 0z"/>
        </svg>
      </div>
      <div class="doc-title">Degree Certificate${multiProg ? '' : ''}</div>
      ${multiProg ? `<div style="font-size:11px;font-weight:700;color:var(--gold);letter-spacing:.5px;margin-bottom:6px;text-transform:uppercase">${lvlLabel} · ${progName}</div>` : `<div style="font-size:12px;color:var(--gold);margin-bottom:6px;">${lvlLabel} — ${progName}</div>`}
      <div class="doc-desc">Official certificate confirming the award of the degree of <strong>${progName}</strong> from the University of Medicine (2)${gradDate ? ', conferred on <strong>' + gradDate + '</strong>' : ''}${gradID ? '. Reference: <strong>' + gradID + '</strong>' : ''}.</div>
      <div class="doc-tags">
        <span class="doc-tag">Official Document</span>
        <span class="doc-tag">Certified</span>
        ${gradDate ? `<span class="doc-tag">${gradDate}</span>` : ''}
      </div>
      <button class="btn-print-doc" onclick="printCertificate(${printIdx})">
        <svg viewBox="0 0 24 24"><polyline points="6,9 6,2 18,2 18,9"/><path d="M6,18H4a2,2,0,0,1-2-2V11a2,2,0,0,1,2-2H20a2,2,0,0,1,2,2v5a2,2,0,0,1-2,2H18"/><rect x="6" y="14" width="12" height="8"/></svg>
        Print Degree Certificate
      </button>
    </div>`;
  }).join('');
}
...
// ── Print Degree Certificate ──────────────────────────────────
function printCertificate(enrollIdx) {
  const enrollments = window._enrollments || [];
  ...
  const father    = document.getElementById('infoFather').textContent.trim();
  const birth     = document.getElementById('infoBirth').textContent.trim();
  const admission = document.getElementById('infoAdmission').textContent.trim();
  const progName  = e.programName || e.degreeProgramId || '';
  const gradDate  = e.graduationDate || '';
  const gradID    = e.graduationId   || '';
  const lvlLabel  = { bachelor:'Bachelor of Medicine, Bachelor of Surgery (MBBS)', master:'Master', phd:'Doctor of Philosophy' }[e.degreeLevel] || progName;
  const today     = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
...
<head>
  <meta charset="UTF-8">
  <title>Degree Certificate — ${esc(name)}</title>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'DM Sans', sans-serif; background: #fff; color: #0D1B2A; font-size: 11pt; line-height: 1.6; }
    @page { margin: 20mm 22mm; size: A4 landscape; }
    @media print { * { -webkit-print-color-adjust: exact; print-color-adjust: exact; } }
    .page {
      width: 100%; max-width: 240mm; margin: 0 auto; padding: 16mm 18mm;
      border: 2.5px solid #9A7B2F;
      min-height: 170mm;
      position: relative;
    ...
    .page::before {
      ...
      position: absolute;
      inset: 5mm;
      border: 1px solid rgba(154,123,47,.25);
      pointer-events: none;
    }
    .logo { width: 64px; height: auto; object-fit: contain; margin-bottom: 10px; }
    .uni-name { font-family: 'DM Sans', sans-serif; font-size: 15pt; font-weight: 700; color: #0D1B2A; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 2px; }
    .uni-sub  { font-size: 9pt; color: #666; margin-bottom: 20px; }
    .cert-title { font-family: 'Libre Baskerville', serif; font-size: 26pt; color: #9A7B2F; margin-bottom: 8px; letter-spacing: -0.5px; }
    .cert-sub   { font-size: 11pt; color: #555; margin-bottom: 28px; }
    .divider { width: 80px; height: 2px; background: linear-gradient(90deg, transparent, #9A7B2F, transparent); margin: 0 auto 28px; }
    .present-text { font-size: 11pt; color: #444; margin-bottom: 14px; }
    .student-name { font-family: 'Libre Baskerville', serif; font-size: 22pt; color: #0D1B2A; border-bottom: 1.5px solid #9A7B2F; padding-bottom: 4px; display: inline-block; margin: 0 auto 8px; min-width: 260px; }
    .degree-text { font-size: 10.5pt; color: #444; margin-bottom: 8px; }
    .degree-name { font-family: 'Libre Baskerville', serif; font-size: 15pt; font-weight: 700; color: #0D1B2A; margin-bottom: 8px; }
    .details { font-size: 9.5pt; color: #555; margin-top: 6px; line-height: 1.9; }
    .footer-row { display: flex; justify-content: space-between; align-items: flex-end; width: 100%; margin-top: auto; padding-top: 28px; border-top: 1px solid rgba(154,123,47,.25); }
    .sig-block { text-align: center; }
    .sig-line  { border-top: 1px solid #0D1B2A; width: 140px; margin: 0 auto 4px; }
    .sig-name  { font-size: 9pt; font-weight: 700; color: #0D1B2A; }
    .sig-title { font-size: 8pt; color: #555; }
    .seal-placeholder { width: 70px; height: 70px; border-radius: 50%; border: 2px solid rgba(154,123,47,.4); display: flex; align-items: center; justify-content: center; font-size: 8.5pt; color: #9A7B2F; text-align: center; line-height: 1.3; font-weight: 700; letter-spacing: .5px; }
    .wm { position: fixed; top: 50%; left: 50%; transform: translate(-50%,-50%) rotate(-35deg); font-size: 60pt; font-weight: 700; color: rgba(154,123,47,.04); pointer-events: none; white-space: nowrap; letter-spacing: 10px; }
    .ref-num { font-size: 8pt; color: #888; position: absolute; top: 8mm; right: 8mm; }
  </style>
</head>
<body>
  <div class="wm">OFFICIAL</div>
  <div class="page">
    ${gradID ? `<div class="ref-num">Ref: ${esc(gradID)}</div>` : ''}
    <img src="https://raw.githubusercontent.com/um2iucoffice/um2portal/cddfb252ec6cf6cecc25c53a8630d92b08c2aaa8/public/UM2_Logo.svg" alt="UM2" class="logo">
    <div class="uni-name">University of Medicine (2)</div>
    <div class="uni-sub">Interim University Council &nbsp;·&nbsp; Republic of the Union of Myanmar</div>

    <div class="cert-title">Certificate of Achievement</div>
    <div class="cert-sub">Degree Award</div>
    <div class="divider"></div>

    <div class="present-text">This is to certify that</div>
    <div class="student-name">${esc(name)}</div>
    <div class="degree-text" style="margin-top:12px">has successfully fulfilled all requirements and is hereby awarded the degree of</div>
    <div class="degree-name">${esc(progName)}</div>

    <div class="details">
      ${esc(lvlLabel !== progName ? lvlLabel : '')}
      ${father && father !== '—' ? `<br>Child of ${esc(father)}` : ''}
      ${birth && birth !== '—' ? `<br>Born: ${esc(birth)}` : ''}
      ${admission && admission !== '—' ? `<br>Admitted: ${esc(admission)}` : ''}
      ${gradDate ? `<br>Date of Conferment: <strong>${esc(gradDate)}</strong>` : ''}
    </div>

    <div class="footer-row">
      <div class="sig-block">
        <img src="signature.png" alt="" style="height:40px;object-fit:contain;opacity:.85;display:block;margin:0 auto 4px">
        <div class="sig-line"></div>
        <div class="sig-name">Kaung Khant</div>
        <div class="sig-title">Acting Registrar</div>
      </div>
      <div class="seal-placeholder">OFFICIAL<br>SEAL</div>
      <div class="sig-block">
        <div class="sig-line"></div>
        <div class="sig-name">University of Medicine (2)</div>
        <div class="sig-title">Vice-Chancellor / Authority</div>
      </div>
    </div>
  </div>
  <script>
...
// ── Override printDocument to use selected enrollment ────────
// Wrap the original printDocument so it picks the right enrollment data.
// We store the original and delegate to it after swapping student globals.
const _origPrintDocument = typeof printDocument === 'function' ? printDocument : null;
