...
<div id="dashboard">
  ...
  <div class="nav">
    <div class="nav-inner">
      ...
      <button class="ntab" data-tab="profile" onclick="switchTab('profile',this)">My Personal Page</button>
      <button class="ntab" data-tab="grades" onclick="switchTab('grades',this)">Academic Record</button>
      <button class="ntab" data-tab="docs" onclick="switchTab('docs',this)">Document Office</button>
...
function populateIDCard(s, isGraduated, gradID, gradDate) {
  ...
  // Only set initials if there's no photo already loaded
  if (idcardPhotoEl && !idcardPhotoEl.querySelector('img')) {
...
function printIDCard() {
  ...
  const s = {
    ...
    init:       (document.getElementById('idcardPhoto').querySelector('img')?.src) || document.getElementById('idcardPhoto').textContent,
    hasPhoto:   !!document.getElementById('idcardPhoto').querySelector('img'),
    isGrad,
    gradID:     _gradID,
    gradDate:   _gradDate,
    idLabel:    _idLabel
  ...
  <div class="idcard">
    <div class="idcard-front">
      ...
      <div class="idcard-divider-line"></div>
      <div class="idcard-body">
        <div class="idcard-photo">${s.hasPhoto ? `<img src="${esc(s.init)}" style="width:100%;height:100%;object-fit:cover">` : esc(s.init)}</div>
        <div class="idcard-info">
...
function printDocument(type) {
  ...
  const program   = document.getElementById('infoProgram')?.textContent || '';
...
const confirmationBody = type === 'confirmation' ? `
  ...
  ">
    ...
    <p style="text-align:justify;">
      ...
      status of <strong>${enroll}</strong>${program && program !== '—' ? ', enrolled in the <strong>' + program + '</strong> program' : ''}.
    </p>
    `}
    <br>
    <p>
...
(function restoreSession() {
  ...
  try {
    populate(saved.studentData, saved.grades || []);
  } catch(e) { return; }
